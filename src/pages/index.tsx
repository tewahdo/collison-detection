...
import { useState, useEffect, useMemo } from 'react';
import { motion } from 'framer-motion';
import { Map, Table2, Plus, Activity, Shield } from 'lucide-react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import SectorForm from '@/components/SectorForm';
import SubmissionMap from '@/components/SubmissionMap';
import SubmissionTable from '@/components/SubmissionTable';
...
export default function Index() {
  ...
  return (
    <div className="bg-background">
      {/* Page header */}
      <div className="container px-4 pt-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            <Map className="h-6 w-6 text-primary" />
            Submissions
          </h1>
          <p className="text-sm text-muted-foreground mt-1">Submit sectors and view collision analysis.</p>
        </div>
        <button
          onClick={() => setShowForm(!showForm)}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors"
      ...
      <main className="container px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Collisions', value: stats.collisions, icon: Shield, color: 'text-collision' },
            { label: 'Pending', value: stats.pending, icon: Activity, color: 'text-pending' },
            { label: 'Approved', value: stats.approved, icon: Activity, color: 'text-approved' },
          ].map(st => (
            <motion.div
              key={st.label}
              initial={{ opacity: 0, y: 10 }}
        ...
        <Tabs defaultValue="map">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            <TabsList>
              ...
              <TabsTrigger value="table" className="gap-1.5"><Table2 className="h-3.5 w-3.5" /> Table</TabsTrigger>
            </TabsList>
            <DashboardFilters filters={filters} onChange={setFilters} />
          </div>
