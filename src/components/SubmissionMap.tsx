...
function FitBounds({ submissions }: { submissions: Submission[] }) {
  ...
  useEffect(() => {
    ...
    const allCoords = submissions.flatMap((s) => {
      // If coordinates array exists
      if (s.coordinates && s.coordinates.length > 0) {
        return s.coordinates.map((c) => [c.lat, c.lng] as [number, number]);
      }
...
// ✅ Color logic
const statusColor = (sub: Submission) => {
  if (sub.hasCollision && sub.status !== "approved") return "#e54545"; // red
  if (sub.status === "approved") return "#2fa87b"; // green
  if (sub.status === "rejected") return "#e54545";
  return "#e89f1a"; // yellow
};
...
export default function SubmissionMap({ submissions = [], onSelect }: Props) {
  return (
    <div className="rounded-lg overflow-hidden border border-border h-[450px] md:h-[550px]">
      <MapContainer
        center={[8.98, 38.79]} // Addis Ababa default
        zoom={11}
        className="h-full w-full"
        scrollWheelZoom
      >
        ...
        {submissions?.map((sub) => {
          ...
          const positions =
            sub.coordinates && sub.coordinates.length > 0
              ? sub.coordinates.map((c) => [c.lat, c.lng] as [number, number])
              : sub.latitude && sub.longitude
                ? [[sub.latitude, sub.longitude] as [number, number]]
                : [];
